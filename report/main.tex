...
\begin{document}
    ...
    \chapter*{ABSTRACT}
    % TODO: Write abstract after implementation is complete.
    This report presents the design and implementation of a statically-typed programming language built as a compiler front-end project. The language supports four primitive types --- Integer, Float, Boolean, and String --- with type inference, arithmetic and boolean expressions, assignment statements, if-then-else and while-loop control flow, function abstraction with value parameter passing, and a built-in \texttt{print()} function. The implementation follows a classical compiler pipeline: lexical analysis, LALR(1) parsing with syntax-directed AST construction, static type checking via a type inference algorithm, and an interpreter for execution. The report provides a detailed description of the context-free grammar and the type inference rules used.
    ...
    \input{1_-_introduction}
    \newpage
    \input{2_-_language_design}
    \newpage
    \input{3_-_system_architecture}
    \newpage
    \input{4_-_implementation}
    \newpage
    \input{5_-_testing}
    \newpage
    \input{6_-_conclusion}
